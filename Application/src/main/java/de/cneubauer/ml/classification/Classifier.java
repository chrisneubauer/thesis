package de.cneubauer.ml.classification;
...
public abstract class Classifier<T, K> implements IFeatureProbability<T, K> {
    ...
    public float featureProbability(T feature, K category) {
        if (this.categoryCount(category) == 0)
            return 0;
        float featureCounter;
        if (this.totalFeatureCount.get(feature) != null) {
            featureCounter = (float) this.totalFeatureCount.get(feature);
        } else {
            return 0;
        }
        float featureInCategory = (float) this.featureCount(feature, category);
        //float featureCount = (float) this.featureCount(feature, category);
        //float categoryCount = (float) this.categoryCount(category);
        // old version with bug:float result = featureCount / categoryCount;
        // new version: if acc structure exists 5 times, abc only 4 times because 5th time is def, the probability for abc is still 1.0
        float result = featureInCategory / featureCounter;
        return result;
    }
