 ...
 * Prototype class to implement generell decision tree algorithm
 */
public class DecisionTreePrototype {
    ...
    protected Node growTree(List<DataSample> trainingData, List<Feature> features, int currentDepth) {
        ...
        if ((currentNodeLabel = getLabel(trainingData)) != null) {
           // log.debug("New leaf is created because data is homogeneous: {}", currentNodeLabel.getPrintValue());
       ...
       // log.debug("Data is split into sublists of sizes: {}", splitData.stream().map(List::size).collect(toList()));
